...
TOPIC_MODEL_PATH = "../Multimodal-Disaster-Classification/topic_modelling/crisis_mmd_topic_model/"
...
class NeuralNetwork(torch.nn.Module):
    def __init__(
        ...
        output_size=len(constants.LABELS_CRISIS_MMD_HUMANITARIAN_CATEGORIES),
    ):
...
def train_bert_topic(docs, y):
    ...
    print()
    for topic in range(len(constants.LABELS_CRISIS_MMD_HUMANITARIAN_CATEGORIES)):
...
if __name__ == "__main__":
    from data.datasets import CrisisMMDDataset
    dataset = CrisisMMDDataset(split_run_type=SplitRunType.TRAIN)
    train_df = dataset._get_dataframe_by_split_run_type()

    texts =  train_df["tweet_text"].tolist()
    humanitarian_category_labels = np.array(train_df["label"].map(
        constants.LABEL_NAME_TO_LABEL_VALUE_CRISIS_MMD_HUMANITARIAN_CATEGORY
    ).tolist())
    texts = [caption_utils.clean_text(t) for t in texts]

    topic_model = get_topic_model(docs=texts, y=humanitarian_category_labels)